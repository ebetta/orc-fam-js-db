...
- **Balance drift is structural, not a bug.** Pluggy keeps 12 months and `pluggy_cutover_date` deliberately excludes hand-entered history, so the local ledger can never reconstruct the balance on its own — and a credit card's invoice cycle keeps moving underneath it. `POST /api/pluggy/reconcile` takes `{ accountId, targetBalance }` and posts one adjustment transaction (`external_id` NULL, so no sync ever deletes it) that aligns `current_balance` to `targetBalance`. **`targetBalance` is always a value the user typed and confirmed** — the endpoint never fetches or trusts Pluggy's own balance figure, because that figure is often stale or mid-cycle. The UI (`PluggySyncTab.jsx`) shows Pluggy's balance only as a pre-filled suggestion in an editable field next to "Saldo real"; the user is expected to check the actual balance on their banking app before saving. For a `CREDIT` account the sensible target is `-balance`, since Pluggy reports the amount owed as positive — the frontend suggests that, but nothing enforces it server-side.
...
- **Manual entries are matched, not blocked.** Before inserting a Pluggy transaction, the sync loop looks for an existing manual transaction (`external_id IS NULL`) in the same account with the same `transaction_type`/`amount` and a `transaction_date` within ±2 days. If found, that row is *adopted* — it gets the Pluggy `external_id` (so future syncs recognize it via the normal `ON CONFLICT` path) but keeps its own description/tag/date untouched. This lets you enter transactions by hand freely without worrying about duplicates; it surfaces in sync results as `status: 'adopted'` (`results.adopted` count), distinct from `imported`/`skipped`. The pre-existing transfer-vs-transfer skip check (same idea, but for `transaction_type = 'transfer'`) runs first and is unaffected.
